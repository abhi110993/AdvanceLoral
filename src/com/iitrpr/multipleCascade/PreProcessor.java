...
public class PreProcessor {
	...
	public void loadServiceCenter() throws IOException{
    	...
    	while((line=br.readLine()) != null) {
    		...
    		ParallelCascadeLoral.serviceMap.put(lineSplit[0],serviceCenter);
    		i++;
    	}
    	br.close();
    }
	...
	public void loadEdges() throws IOException{
    	...
    	while((line=br.readLine()) != null) {
    		...
    		if(!ParallelCascadeLoral.outgoingEdgeMap.containsKey(lineSplit[0])) {
    			HashMap<String,Integer> edgeWeight = new HashMap<String,Integer>();
    			edgeWeight.put(lineSplit[1],Integer.parseInt(lineSplit[2]));
    			ParallelCascadeLoral.outgoingEdgeMap.put(lineSplit[0],edgeWeight);
    		}else {
    			ParallelCascadeLoral.outgoingEdgeMap.get(lineSplit[0]).put(lineSplit[1],Integer.parseInt(lineSplit[2]));
    		}
    		if(!ParallelCascadeLoral.incomingEdgeMap.containsKey(lineSplit[1])) {
    			HashMap<String,Integer> edgeWeight = new HashMap<String,Integer>();
    			edgeWeight.put(lineSplit[0],Integer.parseInt(lineSplit[2]));
    			ParallelCascadeLoral.incomingEdgeMap.put(lineSplit[1],edgeWeight);
    		}else {
    			ParallelCascadeLoral.incomingEdgeMap.get(lineSplit[1]).put(lineSplit[0],Integer.parseInt(lineSplit[2]));
    		}
    	}
    	br.close();
    }
